...
namespace PureMVC.Interfaces
{
    ...
    //model层接口 对 model 注册一个代理（IProxy） 提供 注册 删除 查找 检查 的方法
    ...
    public interface IModel
    {
        ...
        /// <summary>
        /// Register an <c>IProxy</c> instance with the <c>Model</c>  注册代理模式
        /// </summary>
        /// <param name="proxy">A reference to the proxy object to be held by the <c>Model</c></param>
		void RegisterProxy(IProxy proxy);

        /// <summary>
        /// Retrieve an <c>IProxy</c> instance from the Model  通过名字检索一个model的代理实例
        /// </summary>
